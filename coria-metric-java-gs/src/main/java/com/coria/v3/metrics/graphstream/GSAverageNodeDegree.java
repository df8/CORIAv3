 ...
 */
@Component
public class GSAverageNodeDegree extends GSMetricAlgorithmImplementationBase {

    public GSAverageNodeDegree() throws Exception {
        super(AppContext.getInstance().getMetricAlgorithmByName("Average Node Degree").getMetricAlgorithmVariantByName("Default"));
    }
    ...
    public void performComputation(RepositoryManager repositoryManager, DatasetEntity datasetEntity, MetricEntity metricEntity, Map<String, UUID> dependencyMetricIds, List<HashMap<String, String>> parameters) {
        ...
        double result = Toolkit.averageDegree(g);
    ...
    public String toString() {
        return "GSAverageNodeDegree{id: " + getId() + ", name: " + getName() + "}";
    }
}
