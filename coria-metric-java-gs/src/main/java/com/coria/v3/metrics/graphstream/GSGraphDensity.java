 ...
 */
@Component
public class GSGraphDensity extends GSMetricAlgorithmImplementationBase {

    public GSGraphDensity() throws Exception {
        super(AppContext.getInstance().getMetricAlgorithmByName("Graph Density").getMetricAlgorithmVariantByName("Default"));
    }
    ...
    public void performComputation(RepositoryManager repositoryManager, DatasetEntity datasetEntity, MetricEntity metricEntity, Map<String, UUID> dependencyMetricIds, List<HashMap<String, String>> parameters) {
        ...
        double result = Toolkit.density(g);
    ...
    public String toString() {
        return "GSGraphDensity{id: " + getId() + ", name: " + getName() + "}";
    }
}
